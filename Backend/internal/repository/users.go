...
func (r *UserRepository) GetByID(ctx context.Context, id int64) (*models.User, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		SELECT id, TRIM(CONCAT_WS(' ', first_name, last_name)) AS name, email, role, created_at
		FROM users
		WHERE id = ?
...
func (r *UserRepository) GetByEmail(ctx context.Context, email string) (*models.UserWithPassword, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		SELECT id, TRIM(CONCAT_WS(' ', first_name, last_name)) AS name, email, password_hash, role, created_at
		FROM users
		WHERE email = ?
